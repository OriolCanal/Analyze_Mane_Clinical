...
class Vep():
    ...
    def run_vep(self, Vcf, structural_variants=False, custom_gff=False):
        """
        ...
        """
        vcf_file = Vcf.path
        vep_cmd = self.get_vep_cmd(vcf_file, structural_variants, custom_gff)
        ...
        # Ensure the directory containing the VCF file is writable
        vcf_dir = os.path.dirname(vcf_file)

        try:
            env = os.environ.copy()
            Log.info(f"Running vep:\n{joined_cmd}")
            result = subprocess.run(vep_cmd, encoding="utf-8", capture_output=True, env=env)
        ...
        if custom_gff:
            Vcf.set_gff_annotated_vcf(self.get_output_path(vcf_file))
        else:
            Vcf.set_annotated_vcf(self.get_output_path(vcf_file))
        return (self.get_output_path(vcf_file))
    
    def get_vep_cmd(self, vcf_file, structural_variants=False, custom_gff=False):
        """
        ...
        self.vep_cmd = [
            ...
            f"{os.getuid()}:{os.getgid()}", # id -u returns the userID of current user, id -g returns groupid of current user
    ...
    def get_ann_full_path(self, resource):
        """
        joins annotation relative path with the path created for annotation volume in the vep_container
        """
        if hasattr(self.ann_conf, resource):
            resource_obj = getattr(self.ann_conf, resource)
            resource_full_path = resource_obj.file_path
    ...
    def add_custom_gff(self):
        gff_path = self.get_ann_full_path("gencode_gtf")
        gff_vep_path = self.get_ann_vep_path("gencode_gtf")
        print(gff_path)
        if not os.path.isfile(gff_path):
            return(0)
        self.gff_name = "Gencode_gff"
        self.vep_cmd.extend(
            [
                "--custom",
                f"{gff_vep_path},{self.gff_name},gff",
                #"--transcript_filter",
                #f'"_source is Gencode_gff2"'
            ]
        )
